    ...
    MobileArea.prototype = {
        ...
        bindEvent: function() {
            ...
            function popupArea(e) {
                ...
                _self.gearArea.innerHTML = '<div class="area_ctrl slideInUp">' +
                    ...
                    /*'<div class="area_btn ">不在下列选项中的地区我们暂时还不能提供服务，敬请谅解！</div>' +*/
        ...
        setGearTooth: function(data) {
            ...
            if (l > 0) {
                ...
                var itemStr = "";
                for (var i = 0; i < l; i++) {
                    itemStr += "<div class='tooth' e_name='" + item[i]['e_name'] + "'  ref='" + item[i][this.keys['id']] + "'>" + item[i][this.keys['name']] + "</div>";
...
//            _self.trigger.text = provinceText + ((cityText)?(',' + cityText):(''))+ ((countyText)?(',' + countyText):(''));
            $('#addr').text(provinceText + ((cityText)?(',' + cityText):(''))+ ((countyText)?(',' + countyText):('')));
            _self.value = [provinceVal, cityVal, countyVal];
            _self.text = [provinceText, cityText, countyText];
            $("#prov").val(area_province.childNodes[provinceVal].getAttribute('e_name'));
            $("#ccity").val(area_city.childNodes[cityVal].getAttribute('e_name'));
            $("#dist").val(area_county.childNodes[countyVal].getAttribute('e_name'));
            
//             console.log(_self.value);
            if(this.valueTo){
                this.valueTo.value= provinceCode +((cityCode)?(',' + cityCode):('')) + ((countyCode)?(',' + countyCode):(''));
            }
            $('.li_addr').css('border','1px solid #222222');    
            _self.getTime(e);
            _self.close(e);
        },
        close: function(e) {
        ...
        getTime: function(e) {
             var _self = this;
             var sever_time = $('.sever_time');
            $.post( "/index.php?c=Yuyue&a=getAreaBusyDegreeNew",{"prov":_self.text[0],"city":_self.text[1],"dist":_self.text[2]},function(data) {             
                    sever_time.html(data);
//                     console.log(data);
                       if(data==''){
                           $('.time_note').text('我们的客服将尽快跟您确定上门服务时间').show();
                       }else{
                           $('.time_note').hide();
                       }
            },'html');     
            $('.sever_time span').on("click",function(){
                    if($(this).attr('class') != 'nopick'){
                            $('.cur').removeClass('cur').addClass('norm');					
                            $(this).removeClass('norm').addClass('cur');
                    }
            })
            sever_time.show();
        ...
        Gps: function(addr) {
            ...
            var provinceVal,provinceText,provinceCode,cityVal,cityText,cityCode,countyVal,countyText,countyCode = '';
            
            var reg = /省|市/g; //把省和市的后缀去掉
            prov = prov.replace(reg,'');
      
            $( _self.data).each(function(index,item){ 
                if(prov == item['e_name']){
                    provinceVal = parseInt(index);
                    provinceCode = parseInt(item['id']);
                    provinceText =  item['e_name'];
                    provinceTextEng =  item['name'];
                    $(item['child']).each(function(index1,item1){
                        $(item1['child']).each(function(index2,item2){
                            if(district == item2['e_name']){
                                cityVal = parseInt(index1);
                                cityCode = parseInt(item1['id']);
                                cityText =  item1['e_name'];
                                cityTextEng =  item1['name'];
                                countyVal = parseInt(index2);
                                countyCode = parseInt(item2['id']);
                                countyText =  item2['e_name'];
                                countyTextEng =  item2['name'];
                            }
                        });
                    });
                }
           ...
           if(countyText==''||countyText==null){
                $('.time_note').text('您所在的区域不在服务范围').show();
                $('#addr').text('');
                _self.value = [0, 0, 0];
                _self.text = [0, 0, 0];
                $("#prov").val('');
               ...
               $('.time_note').hide();
                $('#addr').text(provinceTextEng + ((cityTextEng)?(',' + cityTextEng):(''))+ ((countyTextEng)?(',' + countyTextEng):('')));
                _self.value = [provinceVal, cityVal, countyVal];
                _self.text = [provinceText, cityText, countyText];
                $("#prov").val(provinceText);
                $("#ccity").val(cityText);
                $("#dist").val(countyText);
                if(this.valueTo){
                    this.valueTo.value= provinceCode +((cityCode)?(',' + cityCode):('')) + ((countyCode)?(',' + countyCode):(''));
